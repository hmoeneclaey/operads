module NonFunctorialOperad where
...
record NfOperad {k} (P : ℕ → Set k) : Set k where
  ...
  field
    ...
    Nfγ : {n : ℕ} → P n → {S : Fin n → ℕ} → ((k : Fin n) → P (S k)) → P (finiteSum S)
    ...
    NfAssoc : {n : ℕ} (c : P n) {S : Fin n → ℕ} (d : (k : Fin n) → P (S k))
              {Q : FinΣ S → ℕ} (e : (x : FinΣ S) → P (Q x))
